...
struct PPagerankProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &v, graphchi_context &ginfo) {
        ...
        if (ginfo.iteration == 0) {
            ...
            if( v.id() == source )
                ...
                pagerank = (1 - RANDOMRESETPROB) * sum;
            //if( pagerank > 0 ) 
            //std::cout << v.id() << " " << pagerank << std::endl;
...
/**
  ...
  */
struct PPagerankProgramInmem : public GraphChiProgram<VertexDataType, EdgeDataType> {
    
    std::vector<EdgeDataType> pr;
    PPagerankProgramInmem(int nvertices) :   pr(nvertices, RANDOMRESETPROB) {}
...
int main(int argc, const char ** argv) {
    ...
    /*if (inmemmode) {
        ...
        //PPagerankProgramInmem program(engine.num_vertices());
