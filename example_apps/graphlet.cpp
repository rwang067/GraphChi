...
struct GraphletProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    uint32_t cnt_all=0;
    uint32_t cnt_ok=0;
    int iter;
            ...
            for(int i=0; i < vertex.num_inedges(); i++) {
                ...
                for(int j=0; j < invector->size(); j++) {
                    ...
                    if (outedge != NULL) {
                        ...
                        if (gcontext.iteration+1 == iter){
                            cnt_all++;
                            if (outedge->vertex_id() == walk){
                                cnt_ok++;
                            }
                        }
                        else{
...
int main(int argc, const char ** argv) {
    ...
    int niters           = get_option_int("niters", 3); // Number of iterations
